...
Sortarea reprezintă una dintre operațiile fundamentale în informatică,
cu aplicații directe în baze de date, motoare de căutare, algoritmi de 
compresie și procesarea datelor la scară largă. Conform estimărilor 
clasice, o parte semnificativă din timpul de calcul al sistemelor reale 
este dedicată operațiilor de ordonare a datelor \cite{knuth1998sorting}.
Alegerea unui algoritm inadecvat poate dubla sau tripla consumul de 
resurse pe date reale față de date artificiale, ceea ce face studiul 
comparativ pe distribuții diverse esențial atât din perspectivă 
academică, cât și practică \cite{bentley1993engineering}.
...
Lucrarea este organizată astfel:Secțiunea~\ref{sec:DateIntrare} definește datele și distribuțiile sortate, Secțiunea~\ref{sec:algoritmi} descrie 
algoritmii, Secțiunea~\ref{sec:ipoteze} prezintă ipotezele teoretice verificate, Secțiunea~\ref{sec:rezultate} analizează rezultatele, 
...
\begin{table}[H]
    ...
    \begin{tabular}{lllll}
    \toprule
    \textbf{Algoritm} & \textbf{Best} & \textbf{Average} & \textbf{Worst} & \textbf{Spațiu} \\
    \midrule
...
sunt evaluate empiric în secțiunea următoare, unde vom vedea dacă Sleep Sort se trezește la înălțimea așteptărilor sau dacă rămâne adormit în fața concurenței.
...
\label{sec:rezultate}
    ...
    \par Măsurările obținute au fost măsurate folosind funcția \texttt{std::chrono::high\_resolution\_clock}, care oferă o rezoluție suficient de fină pentru a evalua performanța algoritmilor, în special pe șirurile mici și medii.
     Am ales această metodă pentru rezoluția sa fină, necesară în special pentru șirurile mici, cu mențiunea că rezultatele pot varia în funcție de încărcarea curentă a sistemului și de prioritățile thread-urilor.
     Pentru a minimiza această variabilitate, testurile au avut loc doar când sistemul era în stare de repaus, fără alte aplicații care să ruleze în fundal, și au fost rulate de sute de ori pentru fiecare combinație de algoritm și distribuție, luând media rezultatelor pentru a obține o estimare mai stabilă a performanței.
     \par Sleep Sort a influențat această decizie deoarece, dacă am fi măsurat timpul bazat pe cicluri de procesor, Sleep Sort ar sorta orice șir instantaneu, și am ieși cu ideea că am revoluționat problema sortării,un vis pentru orice informatician.
...
\par Studiile experimentale riguroase care compară mai mulți algoritmi 
de sortare pe distribuții variate de date sunt surprinzător de rare 
în literatura peer-reviewed. Majoritatea lucrărilor se concentrează 
pe un singur algoritm sau pe un aspect specific,ca eficiența cache.

\par Cea mai apropiată lucrare de abordarea acestui studiu este 
Rai et al. \cite{rai2025comparative}, o lucrare recentă care, deși 
nepeer-reviewed la momentul redactării acestui articol, realizează 
o analiză experimentală detaliată a patru algoritmi clasici, 
Bubble Sort, Quick Sort, Merge Sort și Heap Sort, pe seturi de 
date reale, raportând timpi de execuție cu bare de eroare, număr 
de comparații și swap-uri, și distribuția varianței pe mai multe 
rulări.

\par Această profunzime analitică depășește ce este prezentat 
în această lucrare,totuși, există diferențe metodologice semnificative. 
Rai et al. nu testează distribuțiile \textit{Aproape Sortat} 
și \textit{Plat}, tocmai scenariile în care algoritmi precum 
Insertion Sort sau Quick Sort prezintă comportamente neașteptate 
și care sunt relevante pentru date reale.
