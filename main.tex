...
\section{Rezultate}
    ...
    \par De asemenea, codul sursă folosit pentru testarea algoritmilor și generarea datelor, precum și datele brute,sunt disponibile pe GitHub \ref{sec:GitHub Repository}, pentru a asigura transparența și reproducibilitatea rezultatelor.
...
\subsection{Insertion Sort - Analiză}
    ...
    \par Totuși, pe șirurile mari, performanța sa a fost inferioră comparativ cu cea a altor algoritmi mai eficienți, la fel excluzând-ul din testele pe șirurile de dimensiuni mari.
...
\subsection{Merge Sort - Analiză}
    ...
    \par Un efect surprinzător este saltul în timp de execuție pe șirurile de tipul "Plat",unde timpul de sortare sare semnificativ (relativ cu mărimea șirului). Acest lucru se poate datora urmatorilor 2 factori:
    ...
    Deși a fost mai lent decât std::sort, performanța sa a fost rezonabilă, având în vedere că este o implementare clasică fără optimizări specifice.Iar performanța foarte stabilă pe toate distribuțiile și dimensiunile șirurilor confirmă robustețea sa în fața diferitelor tipuri de date,sortând 2 milioane valori fără .
...
\subsection{Quick Sort - Analiză}
    ...
    Comparativ cu Merge Sort, Quick Sort a avut nevoie de mai puțin overhead,astfel performând mult mai bine pe șirurile mici,dar mai slab față de algoritmele non-recursive.
...
\subsection{Sleep Sort - Analiză}
    ...
    \par Timpul de citire e practic neglijabil comparativ cu timpul de așteptare, deci nu are sens să-l măsurăm în funcție de $n$,acesta fiind relevant doar pentru compararea cu algoritmii clasici.
    ...
    \end{figure}
        ...
        \par Putem observa că timpul de sortare, în conformitate cu așteptările, a crescut semnificativ pe măsură ce valorile elementelor au crescut, confirmând dependența sa de valorile din șir,totuși viteza nu e impractică, considerând independența sa față de $n$.
...
\end{table}
    ...
    \par După cum putem observa,Sleep Sort s-a trezit la înălțimea așteptărilor, depășind atât Merge Sort, cât și std::sort pe toate cele cinci distribuții de date, fiind de 2 ori mai rapid decât Merge Sort și de aproximativ 1.4 ori mai rapid decât std::sort, confirmând avantajul său semnificativ în aceste condiții,
        dovedind că secretul performanței e, la urma urmei, un somn bun.
...
\section{Comparație cu literatura actuală}
  ...
  \par *Merge Sort a confirmat complexitatea $O(n \log n)$
 $O(n\log n)$ în toate cazurile, dar a avut o performanță mai slabă decât Quick Sort și pe șirurile de tipul "Plat" de mărime mică, ceea ce poate fi atribuit mai multor factori discutați în subsecțiunea de analiză a acestuia.

\par Quick Sort a degradat exact unde teoria prezice,pe date sortate și reverse-sortate,confirmând analiza clasică \cite{hoare1961quicksort},și a eșuat pe șirurile mari din aceste distribuții din cauza alegerii ineficientă a pivotului, așa cum era de așteptat.

\par   std::sort a performat conform descrierii Introsort din Musser \cite{musser1997introsort}, inclusiv comportamentul superior pe șiruri aproape sortate față de Quick Sort pur,cât și pe șiruri mici,fără necesitatea a unui overhead de recursivitate.
...
\par Am adus o contribuție în litearura de specialitate și o implementare optimizată a unui algoritm de sortare,ceea ce poate fi rezumat printr-o singură propoziție: "Somnul este esențial pentru performanță, chiar și în sortare."

\section*{Github Repository}
\addcontentsline{toc}{section}{Github Repository}

    Pe acest Repository Github este disponibil codul sursă a tuturor algoritmilor, precum și scripturile de testare,fișierul Excel și datele brute folosite pentru a genera rezultatele din această lucrare.
